...
def autoover():
    ...
    tr=len(alldata)+1
    overtb.insert(0,tr)
    #overtb.configure(state='readonly')
    overtb.configure(state='disabled')
def clearbox():
    overtb.delete(0,"end")
    runstb.delete(0,"end")
    overtb.focus()
def insertfun():
    overtb.configure(state='normal')
    over=int(overtb.get())
    runs=int(runstb.get())
    #print(over,runs)
    dbcon()
...
def delover():
    dbcon()
    window.withdraw()
    overno=simpledialog.askinteger(title="Delete Operation", prompt="Enter Over Number")
    #overno=int(overtb.get())
    #cur.execute("delete from score where over=%d"%overno)
    #cur.execute("delete from score where over=10")
    cur.execute("delete from score where over=%d"%overno)
    con.commit()    
    showchart()
    os.system("databasechart_4_delete.py")
...
but3=tkinter.Button(window,text="DELETE",command=delover)
